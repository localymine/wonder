...
<?= $this->escaper->escapeHtml($this->flash->output()) ?>
      ...
      <div class="box">
        <div class="box-header with-border">
          ...
          <?= $this->tag->linkTo(['servicer/agentboxes/new', $this->l10n->_('<i class="fa fa-file"></i><span>New</span>'), 'class' => 'btn btn-primary btn-sm xcenter']) ?>
...
<?= $this->partial('partials/paginator/agentboxes') ?>

        </div>
      </div>

<?= $this->partial('partials/filter/agentboxes') ?>
...
$(function() {
  ...
  $.confirmationform({
      ...
      childrenPath:'<?= $this->url->get('servicer/agentboxes/exportcsv') ?>',
  ...
  $(document).on('click', 'button[data-aboxid]:not(.disabled)', function(evt) {
    ...
    var options = {
      title:'<?= $this->l10n->_('Schedule reboot Agentbox') ?>'
    };
    switch(state) {
      case 1:
      case 2:
        options.message = String('<?= $this->l10n->_('Do you want to reboot this Agentbox <strong>%s</strong>?<br>This cannot be undone.') ?>').replace('%s', aboxname);
        options.buttons = {
          cancel:{
            label:'<?= $this->l10n->_('<i class="fa fa-times"></i> Cancel') ?>'
          },
          confirm:{
            label:'<?= $this->l10n->_('<i class="fa fa-check"></i> Confirm') ?>'
          }
        };
      ...
      break;
      case 3:
        options.message = String('<?= $this->l10n->_('The Agentbox <strong>%s</strong> is already reboot-scheduled.') ?>').replace('%s', aboxname);
        options.buttons = {
          cancel:{
            label:'<?= $this->l10n->_('<i class="fa fa-times"></i> Cancel') ?>'
          }
      ...
      break;
      case 4:
        options.message = String('<?= $this->l10n->_('The Agentbox <strong>%s</strong> is rebooting.<br>Please wait a minutes.') ?>').replace('%s', aboxname);
        options.buttons = {
          cancel:{
            label:'<?= $this->l10n->_('<i class="fa fa-times"></i> Cancel') ?>'
          }
    ...
    if (evt.keyCode === 18) {
      $('.meta-href').addClass('enable').tooltip({title:'<?= $this->l10n->_('Go to Agentbox Administration login') ?>'});
    }
  }).keyup(function(evt) {
