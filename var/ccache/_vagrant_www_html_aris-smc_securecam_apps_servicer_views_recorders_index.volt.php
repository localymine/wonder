...
<?= $this->escaper->escapeHtml($this->flash->output()) ?>
      ...
      <div class="box">
        <div class="box-header with-border">
          ...
          <?= $this->tag->linkTo(['servicer/recorders/new/', $this->l10n->_('<i class="fa fa-file"></i><span>New</span>'), 'class' => 'btn btn-primary btn-sm xcenter']) ?>

        </div>
        <div class="box-body" data-csrftoken="<?= $this->security->getToken() ?>">

<?= $this->partial('partials/paginator/recorders') ?>

        </div>
      </div>

<?= $this->partial('partials/filter/recorders') ?>
